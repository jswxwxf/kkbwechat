...
class DriveScoreLoader {
  ...
  load() {
    ...
    this.scope.driveScoreOptions = this._options;
  ...
  private _loadChartData(chartData) {
    var labels = _.fill(Array(chartData.scores.length), '');
    this.scope.driveScoreChartData = {
      labels,
      datasets : [
        {
...
export class Loader extends DriveScoreLoader {}